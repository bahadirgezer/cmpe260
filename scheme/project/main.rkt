...
; 4: 20 points
(define split_at_delim (lambda (delim args) (sasad delim args '() '())))

; sasad means split_and_spit_at_delim
(define sasad (lambda (delim to_split group spitted) (cond	
		[(empty? to_split) 
	   		(append spitted (cons group '()))]
		
		[(eq? (first to_split) delim) 
	   		(sasad delim (rest to_split) '() (append spitted (cons group '())))]
	   
		[else (sasad delim (rest to_split) 
			(append group (list(first to_split))) spitted)]
		)
	)	
)
...
(define parse_expr (lambda (expr) (cond
		[(and (list? expr) (ormap (lambda (token) (eq? token '+)) expr)) 
;			(foldl (lambda (org elem) (if (not (list? org)) (append elem (list org)) (append elem (list org)))) '(+) (map parse_expr (remove empty (split_at_delim '+ expr))))]
			(foldl (lambda (org elem) (if  (and (list? org) (= 1 (length org)) (or (number? (car org)) (symbol? (car org)))) (append elem org) (append elem (list org)))) 
				'(+) (map parse_expr (remove empty (split_at_delim '+ expr))))] 

		[(and (list? expr) (ormap (lambda (token) (eq? token '*)) expr)) 
;			(foldl (lambda (org elem) (append elem (list org))) '(*) (map parse_expr (remove empty (split_at_delim '* expr))))]
			(foldl (lambda (org elem) (if  (and (list? org) (= 1 (length org)) (or (number? (car org)) (symbol? (car org)))) (append elem org) (append elem (list org)))) 
				'(*) (map parse_expr (remove empty (split_at_delim '* expr))))]

		[(and (not (empty? expr)) (list? (car expr)) (if (and (< 2 (length expr)) (eqv? (cadr expr) '@)) #f #t)) 
			(let ([f_elem (parse_expr (car expr))] [r_elem (parse_expr (cdr expr))])
				(if (list? f_elem)
					(if (empty? r_elem) f_elem (list f_elem r_elem))
					(if (empty? r_elem) (list f_elem) (list (list f_elem) r_elem))
				))]

		[(and (list? expr) (< 2 (length expr)) (eqv? (cadr expr) '@)) 
					(let ([f_elem (binding_list (car expr))] [r_elem (parse_expr (cddr expr))])
						(if (and (= 1 (length r_elem)) (or (number? (car r_elem)) (symbol? (car r_elem)))) 
							(append f_elem r_elem)
							(append f_elem (list r_elem))
						))]

		[(and (list? expr) (empty? expr)) '()]

		[(and (not (list?  (car expr))) (or (symbol? (car expr)) (number? (car expr))))
			(car expr)]

		[(and (list? expr) (andmap (lambda (token) (or (number? token) (symbol? token))) expr))
			expr]
...
(define assignm (lambda (a_list)
		(foldr (lambda (single_assign org) 
			(cons (:= (eval (car single_assign))(eval (caddr single_assign))) org)) '() a_list)
	)
)

(define binding_list (lambda (b_list) (let* ([clean_list (remove empty (split_at_delim '-- b_list))] [assigned_list (-- (assignm clean_list))]) 
		(list-set assigned_list 1 (first (second assigned_list))));remove empty
	)
)
